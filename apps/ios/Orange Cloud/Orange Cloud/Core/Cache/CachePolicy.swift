...
enum CachePolicy {
    ...
    static func zonesFresh(accountId: String) -> Bool {
        let cached = CacheStore.shared.zones(for: accountId)
        guard !cached.isEmpty else { return false }
        return isFresh(cached.map(\.updatedAt).max(), ttl: zones)
    }

    /// 某域名缓存的 DNS 记录是否仍在有效期内
    static func dnsFresh(zoneId: String) -> Bool {
        let cached = CacheStore.shared.records(for: zoneId)
        guard !cached.isEmpty else { return false }
