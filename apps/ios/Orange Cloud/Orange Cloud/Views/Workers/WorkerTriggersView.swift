...
struct WorkerTriggersView: View {
    ...
    init(accountId: String, scriptName: String, session: SessionStore) {
        _viewModel = StateObject(wrappedValue: WorkerTriggersViewModel(
            ...
            if !viewModel.loaded && viewModel.isLoading {
                ...
                List {
                    Section {
                        ...
                        Text(canWrite
                             ? AppLocalization.string(localized: "Cron 按 UTC 时区执行。向左滑动删除。")
    ...
    private var emptyState: some View {
        OCContentUnavailableView {
            ...
            Text(canWrite
                 ? AppLocalization.string(localized: "添加 Cron 表达式，让这个 Worker 按计划自动运行。")
                 : AppLocalization.string(localized: "该 Worker 未配置 Cron 触发器。"))
        } actions: {
            if canWrite {
            ...
            Form {
                Section {
                    ...
                    Text(isValid
                         ...
                         : AppLocalization.string(localized: "标准 5 字段：分 时 日 月 周（UTC）。"))
...
nonisolated enum CronDescriber {
    static func describe(_ cron: String) -> String {
        ...
        guard f.count == 5 else { return AppLocalization.string(localized: "自定义 Cron（UTC）") }
        let (minute, hour, dom, month, dow) = (f[0], f[1], f[2], f[3], f[4])

        // 每 N 分钟
        if minute.hasPrefix("*/"), hour == "*", dom == "*", month == "*", dow == "*",
           let n = Int(minute.dropFirst(2)) {
            return AppLocalization.string(localized: "每 \(n) 分钟")
        }
        // 每小时整点
        if minute == "0", hour == "*", dom == "*", month == "*", dow == "*" {
            return AppLocalization.string(localized: "每小时整点")
        }
        // 每天 HH:MM（UTC）
        if let m = Int(minute), let h = Int(hour), dom == "*", month == "*", dow == "*" {
            return AppLocalization.string(localized: "每天 \(String(format: "%02d:%02d", h, m))（UTC）")
        }
        return AppLocalization.string(localized: "自定义 Cron（UTC）")
    }
}
